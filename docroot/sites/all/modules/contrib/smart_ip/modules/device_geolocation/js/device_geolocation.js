Drupal.behaviors.deviceGeolocationAutoDetect = function (context) {
  var geolocation_source = 1; // Default it to Maxmind

  if (!Drupal.settings.device_geolocation.ask_geolocate) {
    // Don't ask user for geolocation. Duration of frequency checking is set.
    return;
  }
  Drupal.settings.device_geolocation.ask_geolocate = false;
  if (isset(Drupal.settings.device_geolocation.longitude)) {
    longitude = !isNaN(Drupal.settings.device_geolocation.longitude) ? Drupal.settings.device_geolocation.longitude : (!isNaN(Drupal.settings.device_geolocation.longitude[0]) ? Drupal.settings.device_geolocation.longitude[0] : null);
  }
  else {
    longitude = null;
  }
  if (isset(Drupal.settings.device_geolocation.latitude)) {
    latitude = !isNaN(Drupal.settings.device_geolocation.latitude) ? Drupal.settings.device_geolocation.latitude : (!isNaN(Drupal.settings.device_geolocation.latitude[0]) ? Drupal.settings.device_geolocation.latitude[0] : null);
  }
  else {
    latitude = null;
  }
  // Try W3C Geolocation (Preferred) to detect user's location
  if (navigator.geolocation && !Drupal.settings.device_geolocation.debug_mode) {
